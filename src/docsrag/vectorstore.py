...
from qdrant_client.models import (Distance, Fusion, FusionQuery, Modifier, PointStruct, Prefetch, SparseVector, SparseVectorParams, VectorParams,)
...
def upsert_chunks(chunks: list[Chunk], dense_vectors: list[list[float]], sparse_vectors: list[SparseVec]) -> int:
  points = [
    PointStruct(id=chunk.id, vector = {DENSE: dense, SPARSE: SparseVector(indices=indices, values=values),}, payload = chunk.model_dump())
...
def hybrid_search(dense_vector: list[float], sparse_vector: SparseVec, limit: int) -> list[tuple[Chunk, float]]:
